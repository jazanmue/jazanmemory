...
async function saveItem(e){
  ...
  const item={
    ...
    desc:f('desc').value,
    src,
    media:f('type').value==='فيديو'?'video':'image'
  };
  let items=getItems();
...
function renderAdmin(){
  ...
  let items=getItems();
  tbody.innerHTML=items.map(x=>`<tr><td>${x.media==='video'?'<span class="smallImg videoMark">🎥</span>':`<img class="smallImg" src="${x.src}">`}</td><td>${x.title}</td><td>${x.type}</td><td>${x.category}</td><td>${x.year||''}</td><td><button class="btn dark mini" onclick="editItem('${x.id}')">تعديل</button> <button class="btn mini" onclick="deleteItem('${x.id}')">حذف</button></td></tr>`).join('')
}
function editItem(id){const x=byId(id);if(!x)return;itemId.value=x.id;type.value=x.type;category.value=x.category;title.value=x.title;year.value=x.year;place.value=x.place;desc.value=x.desc;window.scrollTo({top:0,behavior:'smooth'});toast('يمكنك الآن تعديل البيانات ثم الحفظ')}
...
function resetAll(){if(confirm('سيتم استرجاع البيانات الافتراضية وحذف إضافات هذا المتصفح. هل أنتِ متأكدة؟')){resetItems();renderAdmin();toast('تم استرجاع الوثائق والصور الافتراضية')}}
